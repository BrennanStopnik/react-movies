...
const Header = () => {
	return (
		<div>
			...
			<p>An App to call up some movie info from a list.</p>
			<hr/>
			<br/>
		</div>
	)
...
const Footer = () => {
	return (
		<div>
			<br/>
			<p>This site has been brought to you by:</p>
			<h4>Brennan Stopnik</h4>
		</div>
	)
}
...
const MovieInfo = () => {
	return (
		<div>
			{sampleMovies.map(({imdbID, Title, Year, Rated, Director, Writer, Actors, Plot, imdbRating, Images}) => {
				return (
					<>
						<div className='each-movie' key={imdbID}>
							...
							<img className='pics' src={Images[0]}/>
						</div>
						<div>
							<hr/>
						</div>
